...
- PDF는 `pdftotext`로 읽어 자기서사와 경력 전환 신호를 추출했다.
- 소설 초안 안의 외부 링크는 실제 페이지를 확인해 프로젝트 문서에 짧은 요약으로 반영했다.
- Google Photos 링크는 URL과 캡션만 보존했고, 실제 이미지 내용은 아직 별도 해석하지 않았다.
...
- 두 이력서는 동일 인물의 다른 버전으로 보이며, 기간과 직함 표기에 차이가 있다.
- 차이가 있는 항목은 하나의 사실로 확정하지 않고 `버전 차이`로 위키에 기록했다.
...
- 네 개의 구글 슬라이드는 하나의 강연 시리즈로 보인다.
- 각 슬라이드는 원문 전체 대신 핵심 주제와 발췌를 담은 스냅샷 형태로 보존했다.
...
- 처음에는 원문 링크 읽기에 실패했지만, 이후 접근이 가능해져 GM 룰북 내용을 다시 반영했다.
- 이 문서는 추상 세계관 메모보다 `아이들과 바로 플레이 가능한 추리형 TRPG 시나리오 초안`으로 보는 것이 더 정확하다.
...
- `Wiki/Questions`
  - [헤일메리-프로젝트-독후감.md](<../Wiki/Questions/헤일메리-프로젝트-독후감.md>)
- `Wiki/Self`
  - [TODO.md](<../Wiki/Self/TODO.md>)
...
- 이번 입력은 명시적인 `DONE:`은 아니므로 TODO를 닫지 않고 `초안이 생겼다`는 근거만 추가했다.
- 독후감 초안은 단순 원문 보관으로 끝내지 않고, `몰입 지점`과 `후속 질문`을 분리한 질문 노드로 승격했다.
- 오늘 포커스는 같은 TODO를 유지하되, 새 초안 파일을 근거로 다시 연결했다.
...
- `Inbox/Telegram`
  - [942071316.md](<../Inbox/Telegram/2026-04-13/942071316.md>)
  - [942071317.md](<../Inbox/Telegram/2026-04-13/942071317.md>)
  - [942071318.md](<../Inbox/Telegram/2026-04-13/942071318.md>)
  - [942071319.md](<../Inbox/Telegram/2026-04-13/942071319.md>)
...
- Telegram의 `[x]`, `[ ]`, `[]`도 TODO 상태 신호로 인정하도록 컴파일 규칙을 넓혔다.
